...
const getTypes = async (type) => {
  ...
  defaultDictType.value = data.type;
  getList();
};
...
const resetQuery = () => {
  ...
  queryParams.value.type = defaultDictType.value;
  handleQuery();
};
/** 新增按钮操作 */
const handleAdd = () => {
...
const handleUpdate = async (row) => {
  reset();
  const dictCode = row?.code || ids.value[0];
  const res = await getData({code: dictCode});
...
const submitForm = () => {
  dataFormRef.value?.validate(async (valid) => {
    if (valid) {
      form.value.code ? await updateData(form.value) : await addData(form.value);
...
const handleDelete = async (row) => {
  const dictCodes = row?.code || ids.value;
  await proxy?.$modal.confirm('是否确认删除字典编码为"' + dictCodes + '"的数据项？');
  await delData({codes: dictCodes});
  await getList();
...
onMounted(() => {
  getTypes(route.params && (route.params.type as string));
  getTypeList();
});
</script>
