...
export const optionSelect = (roleIds: (number | string)[]): AxiosPromise<RoleVO[]> => {
  return request({
    url: '/api/role/select?roleIds=' + roleIds,
    method: 'get'
  });
};
 ...
 * 角色状态修改
 */
export const changeRoleStatus = (roleId: string | number, status: number) => {
  const data = {
    roleId,
    status
  };
...
export const deptTreeSelect = (roleId: string | number): AxiosPromise<RoleDeptTree> => {
  return request({
    url: '/api/role/deptTree?roleId=' + roleId,
    method: 'get'
  });
};
