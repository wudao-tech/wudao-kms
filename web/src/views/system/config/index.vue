        ...
        <el-card shadow="hover">
          <el-form ref="queryFormRef" :model="queryParams" :inline="true">
            ...
            <el-form-item label="参数键名" prop="code">
              <el-input v-model="queryParams.code" placeholder="请输入参数键名" clearable @keyup.enter="handleQuery" />
            </el-form-item>
...
const handleUpdate = async (row?: ConfigVO) => {
  reset();
  const code = row?.code || ids.value[0];
...
const submitForm = () => {
  configFormRef.value?.validate(async (valid: boolean) => {
    if (valid) {
      form.value.code ? await updateConfig(form.value) : await addConfig(form.value);
...
const handleDelete = async (row?: ConfigVO) => {
  const codes = row?.code || ids.value;
  await proxy?.$modal.confirm('是否确认删除参数编号为"' + codes + '"的数据项？');
  await delConfig(codes);
  await getList();
