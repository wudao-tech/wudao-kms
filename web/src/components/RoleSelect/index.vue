<template>
  <div>
    <el-dialog v-model="roleDialog.visible.value" :title="roleDialog.title.value" width="80%" append-to-body>
      ...
      <el-card shadow="hover">
        <template #header>
          <el-tag v-for="role in selectRoleList" :key="role.id" closable style="margin: 2px" @close="handleCloseTag(role)">
            {{ role.roleName }}
          </el-tag>
        </template>
        ...
        >
          ...
          <vxe-column field="createdAt" title="创建时间" align="center">
            <template #default="scope">
...
const pageList = async () => {
  ...
  const roles = roleList.value.filter((item) => {
    return selectRoleList.value.some((role) => role.id === item.id);
  });
...
const handleCheckboxAll = (checked) => {
  ...
  if (checked.checked) {
    rows.forEach((row) => {
      if (!selectRoleList.value.some((item) => item.id === row.id)) {
        selectRoleList.value.push(row);
      }
    });
  } else {
...
const handleCloseTag = (user: RoleVO) => {
  ...
  // 使用split删除用户
  const index = selectRoleList.value.findIndex((item) => item.id === roleId);
...
const initSelectRole = async () => {
  if (defaultSelectRoleIds.value.length > 0) {
    ...
    const users = roleList.value.filter((item) => {
      return defaultSelectRoleIds.value.includes(String(item.id));
    });
    await nextTick(() => {
