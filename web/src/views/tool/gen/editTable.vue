      ...
      <el-tab-pane label="字段信息" name="columnInfo">
        <el-table ref="dragTable" :data="columns" row-key="columnId" :max-height="tableHeight">
          ...
          <el-table-column label="字典类型" min-width="12%">
            <template #default="scope">
              <el-select v-model="scope.row.category" clearable filterable placeholder="请选择">
                <el-option v-for="dict in dictOptions" :key="dict.category" :label="dict.name" :value="dict.category">
                  <span style="float: left">{{ dict.name }}</span>
                  <span style="float: right; color: #8492a6; font-size: 13px">{{ dict.category }}</span>
                </el-option>
              </el-select>
            </template>
          </el-table-column>
