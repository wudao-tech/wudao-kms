...
export const listArray2Tree = (originList = [], options = {
  key: 'code',
  parentKey: 'parentCode',
  rootValue: null
}) => {
  const {
    key, parentKey, rootValue
  } = options
  const result = []
  const temp = {}
  ...
  for (let item of originList) {
    if (item[parentKey] === rootValue) {
      result.push(temp[item[key]])
    } else {
      if (temp[item[parentKey]]) {
        !temp[item[parentKey]].children && (temp[item[parentKey]].children = [])
        temp[item[parentKey]].children.push(temp[item[key]])
      }
    }
  }
  return result
...
export const nanoid = (length = 20) => {
  const chars = 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789'
  let result = chars[Math.floor(Math.random() * 26)] // 第一位为随机字母
  const lastChart = chars[Math.floor(Math.random() * 10 + 52)] // 最后一位为数组，避免生成关键字
  for (let i = 1; i < length - 1; i++) {
    result += chars[Math.floor(Math.random() * chars.length)] // 其余位为随机字符
  }
  return result + lastChart
}

// 导出数据处理工具函数
export { isEmpty, isNotEmpty, filterEmptyValues, deepFilterEmptyValues, filterEmptyArrayItems } from './dataUtils'


export const getFileTypeFn = (fileName: string) => {
  // if (!fileName) return new URL('../assets/fileIcon/othe.svg', import.meta.url).href;
  
  // 获取文件扩展名
  console.log('iconName',fileName)
