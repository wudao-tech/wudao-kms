<template>
  <div class="p-2">
    <div class="p-card">
      <el-row :gutter="10" class="mb8" style="margin-bottom: 10px; display: flex; justify-content: space-between">
        <div class="dp-c">
          <el-button v-hasPermi="['system:tenantPackage:add']" type="primary" link icon="Plus" @click="handleAdd"> 新增 </el-button>
          <el-button v-hasPermi="['system:tenantPackage:edit']" type="success" link icon="Edit" :disabled="single" @click="handleUpdate()">
            修改
          </el-button>
          <el-button v-hasPermi="['system:tenantPackage:remove']" type="danger" link icon="Delete" :disabled="multiple" @click="handleDelete()">
            删除
          </el-button>
          <el-button v-hasPermi="['system:tenantPackage:export']" type="warning" link icon="Download" @click="handleExport">导出 </el-button>
        </div>
        <el-input
      ...
      <el-table v-loading="loading" :data="tenantPackageList" border :max-height="700" @selection-change="handleSelectionChange">
...
const handleAdd = () => {
  reset();
  getMenuTreeselect();
  console.log(form.value.menuCheckStrictly);
  dialog.visible = true;
  dialog.title = '添加租户套餐';
};
